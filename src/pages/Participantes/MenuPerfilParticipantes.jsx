/* Menu da conta no cabeçalho: o avatar com as iniciais abre um painel com
   a identidade da pessoa e o "sair da conta". Fica no cabeçalho, e não na
   aba Perfil, pra continuar ao alcance de qualquer aba. */

import { useEffect, useRef, useState } from 'preact/hooks';

export default function MenuPerfilParticipantes({ nome, email, iniciais, onSair }) {
    const [aberto, setAberto] = useState(false);
    const containerRef = useRef(null);
    const botaoAvatarRef = useRef(null);
    const itemSairRef = useRef(null);

    /* Só escuta o documento enquanto o menu está aberto — fechado, nenhum
       listener fica pendurado. Abrir também joga o foco no "sair" pra quem
       navega por teclado. */
    useEffect(() => {
        if (!aberto) return undefined;

        itemSairRef.current?.focus();
